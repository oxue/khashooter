...
class LevelLoader {
	...
	private function spawnLights() {
		...
		while (i-- > 0) {
			gc.lightingSystem.addLightSource(new LightSource(levelData.lights[i].x, levelData.lights[i].y,
				levelData.lights[i].color, levelData.lights[i].radius));
		}
	...
	private function createLevelTilemap() {
		ef.createTilemap(levelData.data[0].length, levelData.data.length, levelData.tilesize, 1,
			levelData.data, "all_tiles");
	}
