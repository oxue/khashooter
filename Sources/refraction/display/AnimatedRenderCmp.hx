...
class AnimatedRenderCmp extends Component {
    ...
    public function draw(camera:Camera, ?framebuffer:Framebuffer) {
        time++;
        ...
        if (framebuffer == null) {
            ...
            var theSurface:Surface2D = surface2Set.surfaces[cast coordX + coordY * numRot];
            var bb:Image = ResourceFormat.atlases.get("all").image;
            framebuffer.g2.drawSubImage(
                ResourceFormat.atlases.get("all").image,
                0,
                0,
                theSurface.vx1*bb.width, theSurface.vy1*bb.height, theSurface.width, theSurface.height
            );
        }
    }
}
