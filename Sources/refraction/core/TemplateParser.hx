...
class TemplateParser
{
	...
	public static function parse():StringMap<Dynamic>
	{
		...
		var i:Int = jsonObj.get("entities").length;
	...
	private static function getEntityTemplate(_entityName:String):Dynamic
	{
		if (FALL_BACK_TO_JSON.indexOf(_entityName) == -1) {
			var entityBlobYaml = Reflect.field(Assets.blobs, 'entity_${_entityName}_yaml').toString();
			return Yaml.parse(entityBlobYaml, Parser.options().useObjects());
		}
		var entityBlob:String = Reflect.field(Assets.blobs, 'entity_json_${_entityName}_json').toString();
