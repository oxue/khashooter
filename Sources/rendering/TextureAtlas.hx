...
class ARect {
    ...
    public inline function w():Int {
        return r - l;
    };

    public inline function h():Int {
        return b - t;
    };
}
...
class ANode {
    ...
    public function draw(g:Graphics, assets:Map<Int, Surface2D>, size:Int = -1) {
        ...
        if (image != null) {
            if (TextureAtlas.lastImage == null){
                TextureAtlas.lastImage = image.data;
            }
        ...
        if (child[0] != null)
            child[0].draw(g, assets, size);
        if (child[1] != null)
            child[1].draw(g, assets, size);
    }
...
class TextureAtlas {
    ...
    public var bs:Array<Image>;

    public static var lastImage:Image;
    ...
    public static function bake(_data:Image, _rot:Int):Image {
        ...
        var cache:Image = Image.createRenderTarget(diagnol * _rot, diagnol, null, NoDepthAndStencil, 1);
    ...
    public static function bakeForAnimation(_data:Image, _frame:IntRect, _rot:Int):Image {
        ...
        var ret:Image = Image.createRenderTarget(diagnol * _rot, gWidth * gHeight * diagnol, null, NoDepthAndStencil, 1);
        ret.g2.begin(true, Color.fromFloats(0, 0, 0, 0));
        ret.g2.end();
        var brush:Image = Image.createRenderTarget(_frame.w, _frame.h, null, NoDepthAndStencil, 1);
        var i:Int = -1;
        while (i++ < gHeight - 1) {
            var j:Int = -1;
            while (j++ < gWidth - 1) {
    ...
    public function splitAndIndex(_img:Image, _frame:FloatRect):SurfaceSetCmp {
        ...
        while (i++ < gh - 1) {
            var j:Int = -1;
            while (j++ < gw - 1) {
                var b:Image = Image.createRenderTarget(cast _frame.w, cast _frame.h, null, NoDepthAndStencil);
                _frame.x = j * _frame.w;
                _frame.y = i * _frame.h;


                b.g2.begin(true, Color.Transparent);
    ...
    public function binpack(numRot:Int) {
        ...
        image = Image.createRenderTarget(size, size, null, NoDepthAndStencil, 1);
